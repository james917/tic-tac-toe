...
function GameState(name) {
	...
	canvas.addEventListener("mousedown", function(evt) {
		...
		if (px % 120 >= 20 && py % 120 >= 20) {
			...
			if (mode & ONE_PLAYER) {
				...
				player = player === Tile.NOT ? Tile.CROSS : Tile.NOT;
			}
		}
	}, false);
	...
	this.init = function(_mode, tile) {
		...
		player = tile || Tile.NOT;

		isPlayer = player === Tile.NOT;
		aiMoved = false;
		winner = false;
		winnerMsg = false;
		hastick = false;
		...
		ai.setSeed(player === Tile.NOT ? Tile.CROSS : Tile.NOT);
	...
	this.update = function() {
		...
		if (!activeAnim) {
			...
			if (winner && !aiMoved) {
				...
				} else if (winner === Tile.NOT) {
					winnerMsg = "The Not player won!";
				} else {
...
function AboutState(name) {
	...
	var text = "Tic-tac-toe is a game for players, X and O. Take turns marking the spaces in a 3×3 grid. The player who succeeds in placing three marks in a horizontal, vertical, or diagonal row wins!";
	var hastick = false;
